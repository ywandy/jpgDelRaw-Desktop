...
export function ScanResultPage({
  ...
  return (
    <div className="page">
      <div className="mx-auto flex min-h-0 w-full max-w-[1380px] flex-1 flex-col gap-3 overflow-hidden">
        <section className="page-header shrink-0">
          ...
          <button className="btn btn-secondary shrink-0" onClick={onRescan}>
            <RefreshCw className="h-4 w-4" />
            重新扫描
          </button>
        </section>

        <section className="grid shrink-0 grid-cols-5 gap-2.5">
        ...
        {hasCandidates && (
          <section className="alert-panel alert-orange shrink-0">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--radius-sm)] bg-[#ffdfa8] text-[var(--color-warning-strong)]">
      ...
      <div className="page-actionbar">
        <div className="mx-auto flex w-full max-w-[1380px] flex-wrap justify-end gap-3">
          ...
          <button
            className="btn btn-danger min-w-56"
...
function ResultMetric({ label, value, helper, tone }: { label: string; value: string | number; helper: string; tone: MetricTone }) {
  ...
  return (
    <div className="relative min-w-0 overflow-hidden rounded-[var(--radius-md)] border border-[var(--color-border)] bg-white p-3 shadow-[var(--shadow-panel)]">
      <div className={`absolute -right-6 -top-7 h-16 w-16 rounded-full ${style.wash}`} />
