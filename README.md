...
RAW Pair Cleaner is built for photographers who export JPG-like previews while keeping RAW originals. It scans a selected photo directory, compares files by extensionless lowercase basename, highlights unmatched files according to the selected cleanup mode, and lets the user review every pending deletion before anything is moved.

The app is intentionally conservative: conflict files are never auto-deleted, renderer code does not touch the filesystem directly, and user files are moved to the system trash/recycle bin instead of being hard-deleted.

## Features

- **Two cleanup modes**
  - Use JPG-like files as source of truth and remove unmatched RAW files.
  - Use RAW files as source of truth and remove unmatched JPG-like files.
- **Safe review flow**
  - Shows scan results, matched pairs, conflicts, pending delete candidates, and estimated reclaimable size.
  - Supports unselecting individual delete candidates before execution.
- **Conflict protection**
...
- **Trash-first deletion**
  ...
  - No hard-delete path is used for user photo files.
- **Operation logs**
  - Optional JSON delete logs are written under the Tauri app data directory.
- **Desktop-focused UI**
  - Tauri v2 desktop shell, React renderer, custom title bar, sidebar navigation, and responsive desktop layout.
...
7. Write an operation log when logging is enabled.

## Safety Model

![Safety model](docs/assets/readme-safety-model.svg)

The core safety rules are part of the product contract:
...
The canonical extension lists live in [`shared/fileExtensions.ts`](shared/fileExtensions.ts) and are mirrored by the Rust service types.
...
RAW Pair Cleaner is designed as a local desktop utility. File scanning, comparison, deletion, settings, and logs are handled locally through the Tauri application. The app does not need cloud sync or remote image processing for its core workflow.
...
3. Add focused tests for compare logic, renderer helpers, or backend services when the behavior changes.
