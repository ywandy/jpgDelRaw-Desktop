...
import { IMAGE_DIRECTORY_NAMES, RAW_DIRECTORY_NAMES } from "../../shared/fileExtensions.js";
import { getFileKey, getMediaKind, isHiddenName, normalizeExtension } from "../../shared/fileUtils.js";
import type { DirectoryMode, MediaFile, ScanOptions, ScanResult } from "../../shared/types.js";
...
export function detectDirectoryMode(
  ...
  const imageDirectory = rootDirectoryDetection.jpgDirectory ?? findFirstMatchingDirectory(rootPath, imageFiles, IMAGE_DIRECTORY_NAMES);
  const rawDirectory = rootDirectoryDetection.rawDirectory ?? findFirstMatchingDirectory(rootPath, rawFiles, RAW_DIRECTORY_NAMES);
...
async function detectRootDirectories(rootPath: string, options: ScanOptions): Promise<Partial<DirectoryDetection>> {
...
function findFirstMatchingDirectory(rootPath: string, files: MediaFile[], directoryNames: string[]): string | undefined {
